...
#ifndef SOLIDRECON_APP_VERSION
#define SOLIDRECON_APP_VERSION Error
#endif

#define SOLIDRECON_APP_VERSION_STR STR(SOLIDRECON_APP_VERSION)
...
namespace jana {
  ...
  void PrintUsageOptions() {
    ...
    std::cout << "   --list-available-plugins     List plugins at $JANA_PLUGIN_PATH and $solid_recon_MY"
      << std::endl;
    std::cout << std::endl;
  ...
  void PrintUsageExample() {
    ...
    std::cout << "    solid_recon -Pplugins=plugin1,plugin2,plugin3 -Pnthreads=8 infile.root"
      << std::endl;
    std::cout << "    solid_recon -Ppodio:print_type_table=1 infile.root" << std::endl << std::endl;
    std::cout << std::endl << std::endl;
  }
  ...
  void PrintUsage() {
    ...
    std::cout << "    solid_recon [options] source1 source2 ..." << std::endl;
    std::cout << std::endl;
  ...
  void PrintVersion() { std::cout << "solid_recon version: " << SOLIDRECON_APP_VERSION_STR << std::endl; }
  ...
  void GetPluginNamesFromEnvPath(std::set<std::string>& plugin_names, const char* env_var) {
    ...
    if (env_p) {
      if (strcmp(env_var, "solid_recon_MY") == 0) {
  ...
  std::vector<std::string> GetAvailablePluginNames(std::vector<std::string> const& default_plugins) {
    ...
    jana::GetPluginNamesFromEnvPath(set_plugin_name, "solid_recon_MY");
  ...
  void AddAvailablePluginsToOptionParams(UserOptions& options,
    ...
    // Add the plugins at $solid_recon_MY/plugins.
    //        jana::GetPluginNamesFromEnvPath(set_plugins, "solid_recon_MY");  // disabled as we do not
  ...
  JApplication* CreateJApplication(UserOptions& options) {
    ...
    const char* env_p = getenv("solid_recon_MY");
    if (env_p) {
  ...
  UserOptions GetCliOptions(int nargs, char* argv[], bool expect_extra) {
    ...
    // `solid_recon` has the same effect with `solid_recon -h`
    if (nargs == 1) {
